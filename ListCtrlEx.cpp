...
int CListCtrlEx::FindItem(LVFINDINFO* pFindInfo, int nStart) const
{
	if(pFindInfo->flags & LVIF_PARAM)
	{
		...
		{
			if((DWORD_PTR)pFindInfo->lParam == GetItemData(i)) return i;
		}
		return -1;
	}
	else
	...
	switch(lplvcd->nmcd.dwDrawStage)
	{
		...
		*pResult = CDRF_NOTIFYSUBITEMDRAW;          // ask for subitem notifications.
		...
		case CDDS_ITEMPREPAINT|CDDS_SUBITEM:
			{
				if(m_bHighlight && GetItemState(iRow, LVIS_SELECTED) == LVIS_SELECTED)
				{
...
int CListCtrlEx::CompareInt(LPCTSTR pLeftText, LPCTSTR pRightText)
{
	return (int)(_ttol(pLeftText) - _ttol(pRightText));
}
int CListCtrlEx::CompareDouble(LPCTSTR pLeftText, LPCTSTR pRightText)
{
	return (int)(_ttof(pLeftText) - _ttof(pRightText));
}
int CListCtrlEx::CompareString(LPCTSTR pLeftText, LPCTSTR pRightText)
{
	return CString(pLeftText).Compare(pRightText);
}
int CListCtrlEx::CompareNumberString(LPCTSTR pLeftText, LPCTSTR pRightText)
{
	LONGLONG l1 = _ttol(pLeftText);
	LONGLONG l2 = _ttol(pRightText);
	if(l1 && l2 && (l1 - l2))
	{
		CString str1, str2;
...
int CListCtrlEx::CompareNumberStringNoCase(LPCTSTR pLeftText, LPCTSTR pRightText)
{
	LONGLONG l1 = _ttol(pLeftText);
	LONGLONG l2 = _ttol(pRightText);
	if(l1 && l2 && (l1 - l2))
...
int CListCtrlEx::CompareStringNoCase(LPCTSTR pLeftText, LPCTSTR pRightText)
{
	return CString(pLeftText).CompareNoCase(pRightText);
}
int CListCtrlEx::CompareDate(LPCTSTR pLeftText, LPCTSTR pRightText)
{
	COleDateTime dL, dR;
